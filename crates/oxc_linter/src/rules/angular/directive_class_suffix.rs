...
pub struct DirectiveClassSuffixConfig {
    /// List of allowed suffixes for directive class names.
...
declare_oxc_lint!(
    ...
    pedantic,
    pending,
    config = DirectiveClassSuffixConfig
);

impl Rule for DirectiveClassSuffix {
    fn from_configuration(value: serde_json::Value) -> Result<Self, serde_json::Error> {
        let config =
            serde_json::from_value::<DefaultRuleConfig<DirectiveClassSuffixConfig>>(value)
